...
- 双击设备名称直接修改，回车或失焦保存、Esc 取消；双击设备其他位置展开详情；Shift 点击多选，Shift 拖动空白处框选；空格拖动或中键平移，滚轮缩放。
...
- 选择模式拖动空白处可平移，连线模式拖动任意图元可平移；双击空白处或区域框返回选择。设备库、属性栏靠近画布的边缘竖条可分别收起或展开。
...
- 区域框标题左侧或属性栏可锁定/解锁。锁定后不可选中拖动或缩放，锁定状态随项目保存。选择工具下，空白处及锁定区域框内直接拖动可平移画布；按住 Shift 拖动空白处仍可框选设备。

- 连线模式也支持拖动空白处和锁定区域框平移，并保留已选连线起点。双击空白处或区域框可回到选择工具。

- 连线模式下设备显示手指光标，点击选择连线端点；拖动设备平移整个画布，不改变设备位置或切换工具。

- 连线模式下，从设备、线路、区域框或空白处拖动均平移整个画布，不移动或缩放图中元素；单击设备仍用于连线。

- 单击画布空白处或锁定区域框空白处清除设备、线路选择；拖动平移保留选择。

- 连线模式只允许点击设备作为连线端点，点击已有线路不会选中线路；拖动线路仍平移画布。

- Esc 优先取消设备、线路或连线起点的选择；没有选中项时退出连线模式。有选中项或处于连线模式时，优先处理这些操作，再按 Esc 才退出页面全屏。

- 双击区域框标题文字可直接编辑，回车或失焦保存，Esc 取消；锁定只限制移动和缩放，仍允许修改标题。双击区域框空白处仍回到选择工具。
